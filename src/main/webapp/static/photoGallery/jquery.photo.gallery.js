...
$.fn.extend({
	...
	photoGallery: function(options) {
			...
			//右下角缩略图高度
 	 	 	...
 	 	 	template : {
	 	 	 	...
	 	 	 	OPERTATION : '<div class="oper">' +
							 ...
							 '<div class="tool">' +
							 	'<div class="toolct">' +
								 	'<span class="oper_fullscreen" title="查看全屏"><i class="fa fa-arrows-alt"></i></span>' +
									'<span class="oper_bigger" title="放大图片"><i class="fa fa-search-plus"></i></span>' +
									'<span class="oper_smaller" title="缩小图片"><i class="fa fa-search-minus"></i></span>' +
									'<span class="oper_rotate" title="旋转图片"><i class="fa fa-refresh"></i></span>'/* +
		...
		$prev.bind('click',function(){
	 	...
	 	$thumbnails.find(".thumbClose").bind("click",function(){
	  		$thumbnails.hide();
	  	});
		...
		function mouseWheelScroll(e){
	 	...
	 	function nextImg(){
	 		if(o.activeIndex < o.imgs.length -1){
	 			$next.removeClass("negative");
      		}else{
      			$next.addClass("negative");
      		}
	 	}
		
	 	function prevImg(){
	 		if(o.activeIndex == 0) $prev.addClass("negative");
	 		else $prev.removeClass("negative");
...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		$("<iframe></iframe").appendTo("body")
			...
			.css({
				position : "absolute",
				left : $(".gallerys").offset().left+10,
				top : $(".gallerys").offset().top,
				width : $gallerys.width()+ parseInt($gallerys.css("padding-left"))*2,
				height : $gallerys.height(),
